...
class TableSearcher:
    ...
    def _keyword_matches_table(self, df_str: str, keyword: str, case_sensitive: bool) -> bool:
        """Check if keyword matches in table string"""
        if case_sensitive:
            return keyword in df_str
        return keyword.lower() in df_str.lower()

    def _create_table_match_result(self, table_id: str, table_info: Dict, df: pd.DataFrame) -> Dict:
        """Create result dictionary for matching table"""
        return {'table_id': table_id, 'page': table_info['location'].get('page'),
                'shape': table_info['shape'], 'dataframe': df}

    def _search_single_table(self, table_id: str, table_info: Dict,
                            keyword: str, case_sensitive: bool) -> Optional[Dict]:
    ...
    def _search_all_tables(self, keyword: str, case_sensitive: bool) -> List[Dict]:
        ...
        return [result for table_id, table_info in self.table_loader.tables_data.items()
                if (result := self._search_single_table(table_id, table_info, keyword, case_sensitive))]
    ...
    def _find_matching_rows(self, df: pd.DataFrame, row_keyword: str) -> pd.DataFrame:
        ...
        row_mask = df.astype(str).apply(
            lambda row: row.str.contains(row_keyword, case=False, na=False).any(), axis=1)
        return df[row_mask]

    def _find_column_by_keyword(self, df: pd.DataFrame, col_keyword: str) -> Optional[str]:
        """Find column name matching keyword"""
        col_matches = [col for col in df.columns if col_keyword.lower() in str(col).lower()]
        return col_matches[0] if col_matches else None

    def _extract_first_numeric_value(self, row: pd.Series, df: pd.DataFrame) -> Optional[Any]:
    ...
    def _get_value_by_column(self, row: pd.Series, df: pd.DataFrame, col_keyword: Optional[str]) -> Optional[Any]:
    ...
    def _find_and_validate_rows(self, df: pd.DataFrame, row_keyword: str, table_id: str) -> Optional[pd.DataFrame]:
    ...
    def extract_numerical_value(self, table_id: str, row_keyword: str,
                                col_keyword: Optional[str] = None) -> Optional[Any]:
        ...
        if matching_rows is None: return None
    ...
    def _find_numeric_in_adjacent_cells(self, row: pd.Series, col_name: str) -> Optional[str]:
    ...
    def _create_metric_match_result(self, table_id: str, table_info: Dict,
                                    row_idx, col_name: str, cell_str: str, numeric_value) -> Dict:
        """Create result dictionary for metric match"""
        return {'table_id': table_id, 'page': table_info['location'].get('page'),
                'row_index': row_idx, 'column': col_name,
                'context': cell_str, 'value': numeric_value}

    def _process_metric_cell(self, keyword: str, cell_value: Any, row: pd.Series,
                            col_name: str, row_idx, table_id: str, table_info: Dict) -> Optional[Dict]:
        """Process a single cell for keyword match"""
        cell_str = str(cell_value).lower()
        if keyword.lower() not in cell_str: return None
        ...
        return self._create_metric_match_result(table_id, table_info, row_idx, col_name, cell_str, numeric_value)

    def _extract_metrics_from_table(self, table_id: str, table_info: Dict,
                                   metric_keywords: List[str], results: Dict):
        """Extract metrics from a single table"""
        df = pd.DataFrame(table_info['dataframe'])
        for keyword in metric_keywords:
            for row_idx, row in df.iterrows():
                for col_name, cell_value in row.items():
                    match = self._process_metric_cell(keyword, cell_value, row, col_name, row_idx, table_id, table_info)
                    if match: results[keyword].append(match)
    ...
    def _extract_metrics_from_all_tables(self, metric_keywords: List[str], results: Dict):
        """Extract metrics from all loaded tables"""
        for table_id, table_info in self.table_loader.tables_data.items():
            self._extract_metrics_from_table(table_id, table_info, metric_keywords, results)

    def extract_financial_metrics(self, metric_keywords: List[str]) -> Dict[str, List[Dict]]:
