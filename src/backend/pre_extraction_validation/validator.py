...
class PDFExtractionValidator:
    ...
    def calculate_extraction_metrics(self, pdf_text: str, markdown_text: str) -> Dict:
        ...
        return {'pdf_char_count': pdf_chars, 'markdown_char_count': md_chars,
                'pdf_word_count': pdf_words, 'markdown_word_count': md_words,
    ...
    def _log_pdf_info(self, pdf_metadata: Dict):
        """Log PDF information"""
        self.logger.info(f"  PDF: {pdf_metadata['page_count']} pages, "
    ...
    def _log_extraction_info(self, extraction_metrics: Dict):
        ...
        self.logger.info(f"  Docling: {extraction_metrics['markdown_char_count']:,} chars, "
                        f"{extraction_metrics['markdown_word_count']:,} words")
        self.logger.info(f"  Extraction rate: {extraction_metrics['char_extraction_rate']:.1f}% chars, "
    ...
    def _log_keyword_info(self, markdown_keywords: Dict):
        ...
        self.logger.info(f"  Overall keyword coverage: {coverage:.1f}% ({found}/{total})")

    def _build_validation_report(self, pdf_path: Path, markdown_path: Path, pdf_metadata: Dict,
                                 extraction_metrics: Dict, markdown_keywords: Dict) -> Dict:
        """Build validation report dictionary"""
        return {'company': self.config.company_name, 'pdf_path': str(pdf_path),
                'markdown_path': str(markdown_path), 'pdf_metadata': pdf_metadata,
                'extraction_metrics': extraction_metrics, 'keyword_coverage': markdown_keywords,
                'quality_assessment': self._assess_quality(extraction_metrics, markdown_keywords)}
    ...
    def _resolve_validation_paths(self, pdf_path, markdown_path):
        ...
        return pdf_path or self.config.pdf_path, markdown_path or self.config.markdown_path
    ...
    def _extract_pdf_text_and_metadata(self, pdf_path):
        ...
        self._log_pdf_info(pdf_metadata); return pdf_metadata, pdf_text
    ...
    def _get_extracted_text(self, markdown_path):
        ...
        return merged_text if merged_text else self._load_markdown_fallback(markdown_path)
    ...
    def _calculate_and_log_metrics(self, pdf_text, extracted_text):
        ...
        self._log_extraction_info(extraction_metrics); return extraction_metrics
    ...
    def _check_and_log_keywords(self, extracted_text):
        ...
        self._log_keyword_info(keywords_coverage); return keywords_coverage

    def _build_and_save_report(self, pdf_path, markdown_path, pdf_metadata, extraction_metrics, keywords_coverage):
        """Build and save validation report (Step 5)"""
        validation_report = self._build_validation_report(pdf_path, markdown_path, pdf_metadata,
                                                          extraction_metrics, keywords_coverage)
        return self._save_validation_report(validation_report)

    def validate_extraction(self, pdf_path: Path = None, markdown_path: Path = None) -> Path:
        """Validate Docling extraction quality against original PDF"""
        pdf_path, markdown_path = self._resolve_validation_paths(pdf_path, markdown_path)
        ...
        output_path = self._build_and_save_report(pdf_path, markdown_path, pdf_metadata, extraction_metrics, keywords_coverage)
        self.logger.info(f"✓ Validation complete: {output_path}"); return output_path

    def _determine_quality_level(self, char_rate: float, keyword_rate: float) -> tuple[str, str]:
        """Determine quality level and status"""
        MIN_EXTRACTION_RATE, GOOD_KEYWORD_COVERAGE, FAIR_KEYWORD_COVERAGE = 70, 0.85, 0.60
    ...
    def _add_extraction_rate_issue(self, issues, char_rate, MIN_EXTRACTION_RATE):
        ...
        if char_rate < MIN_EXTRACTION_RATE:
            issues.append(f"Low extraction rate: {char_rate:.1f}% (expected >{MIN_EXTRACTION_RATE}%)")
    ...
    def _add_keyword_coverage_issue(self, issues, keyword_rate, FAIR_KEYWORD_COVERAGE):
        ...
        if keyword_rate < FAIR_KEYWORD_COVERAGE:
            issues.append(f"Low keyword coverage: {keyword_rate*100:.1f}% (expected >{FAIR_KEYWORD_COVERAGE*100}%)")
    ...
    def _add_category_coverage_issues(self, issues, keyword_coverage):
        ...
        for category, data in keyword_coverage.items():
            if category != 'overall' and data['coverage'] < 0.5:
                issues.append(f"Poor coverage in {category}: {data['coverage']*100:.1f}%)")

    def _identify_issues(self, char_rate: float, keyword_rate: float, keyword_coverage: Dict) -> List[str]:
    ...
    def _build_quality_assessment_dict(self, quality, status, char_rate, keyword_rate, issues):
        """Build quality assessment dictionary"""
        return {'quality': quality, 'status': status, 'char_extraction_rate': char_rate,
                'keyword_coverage_rate': keyword_rate * 100, 'issues': issues,
    ...
    def _assess_quality(self, extraction_metrics: Dict, keyword_coverage: Dict) -> Dict:
        ...
        char_rate, keyword_rate = self._extract_rates(extraction_metrics, keyword_coverage)
        ...
        return self._build_quality_assessment_dict(quality, status, char_rate, keyword_rate, issues)
