...
class MarkdownExtractor:
    ...
    def _initialize_components(self):
        ...
        self.chunker = ContentChunker(
            self.logger, self.min_chunk_size, self.max_chunk_size
        )
    ...
    def _has_valid_preamble(self, markdown_text: str, first_header_pos: int) -> bool:
        ...
        return (
            first_header_pos > 0
            and len(markdown_text[:first_header_pos].strip()) >= self.min_chunk_size
        )

    def _extract_preamble(
        self, markdown_text: str, first_header_pos: int, chunk_id: int
    ) -> tuple[List[Dict], int]:
        """Extract preamble content before first header"""
        if not self._has_valid_preamble(markdown_text, first_header_pos):
            return [], chunk_id
    ...
    def _process_section(
        self, headers: List[Dict], index: int, content: str, chunk_id: int
    ) -> tuple[List[Dict], int]:
        ...
        section_chunks = self.chunker.create_chunks_from_content(
            content_chunks, hierarchical_path, headers[index]["level"], chunk_id
        )
        return section_chunks, chunk_id + len(section_chunks)

    def _process_all_sections(
        self, markdown_text: str, headers: List[Dict], chunk_id: int
    ) -> tuple[List[Dict], int]:
        ...
        for i in range(len(headers)):
            ...
            if content and len(content) >= self.min_chunk_size:
                section_chunks, chunk_id = self._process_section(
                    headers, i, content, chunk_id
                )
    ...
    def extract_hierarchical_chunks(self, markdown_text: str) -> List[Dict]:
        ...
        if not headers:
            return self._process_no_headers(markdown_text)
        
        preamble_chunks, chunk_id = self._extract_preamble(
            markdown_text, headers[0]["start_pos"], 0
        )
        section_chunks, _ = self._process_all_sections(markdown_text, headers, chunk_id)
        chunks = preamble_chunks + section_chunks
        self.logger.info(f"Extracted {len(chunks)} chunks with hierarchical structure")
        return chunks
    ...
    def _read_and_log_markdown(self, markdown_path: Path) -> str:
        ...
        self.logger.info(f"Read {len(markdown_text)} characters")
    ...
    def process_markdown_file(
        self, markdown_path: Optional[Path] = None) -> Dict:
...
def _create_cli_parser():
    ...
    parser.add_argument("--markdown", type=str, help="Path to markdown file (optional)")
    parser.add_argument(
...
def _run_extraction(config, args):
    ...
    extractor.process_markdown_file(markdown_path)
