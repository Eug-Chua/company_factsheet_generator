...
class ContentChunker:
    ...
    def _is_chunk_too_small(self, content: str, section_header: str) -> bool:
        ...
        if len(content.strip()) < self.min_chunk_size:
            self.logger.debug(f"Skipping small chunk ({len(content)} chars): {section_header[:50]}")
            return True
        return False

    def create_chunk(self, chunk_id: int, section_header: str, header_level: int, content: str) -> Optional[Dict]:
        """Create a chunk dictionary with validation"""
        if self._is_chunk_too_small(content, section_header): return None
        return {'chunk_id': chunk_id, 'section_header': section_header,
                'header_level': header_level, 'content': content}

    def create_chunks_from_content(self, content_chunks: List[str], section_header: str, level: int, start_id: int = 0) -> List[Dict]:
        ...
        for chunk_content in content_chunks:
            ...
            if chunk: chunks.append(chunk); chunk_id += 1
        return chunks
    ...
    def _collect_table_lines(self, lines: List[str], start_idx: int) -> tuple[List[str], int]:
        """Collect all consecutive table lines"""
        table_lines, idx = [], start_idx
        while idx < len(lines) and self._is_table_row(lines[idx]):
            table_lines.append(lines[idx]); idx += 1
    ...
    def _process_table(self, lines: List[str], i: int, current_chunk: str, chunks: List[str]) -> tuple[str, int]:
        """Process a table and update chunks"""
        table_text, end_idx = self._extract_table(lines, i)
        if current_chunk and len(current_chunk) + len(table_text) + 1 > self.max_chunk_size:
    ...
    def _process_lines_to_chunks(self, lines: List[str]) -> List[str]:
        ...
        while i < len(lines):
            if self._is_table_row(lines[i]):
                ...
                current_chunk = self._process_line(lines[i], current_chunk, chunks); i += 1
        self._finalize_chunks(chunks, current_chunk); return chunks

    def split_content(self, content: str) -> List[str]:
        """Split content into chunks respecting max size, paragraph boundaries, and tables"""
        if len(content) <= self.max_chunk_size: return [content]
